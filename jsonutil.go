...
//将模糊查询的json转化为go数据
func ParseJson2Idioms(jsonStr string) (idiomsMap map[string]main2.Idiom) {
	idiomsMap = make(map[string]main2.Idiom)
...
//将精确查询的json转化为go数据
func ParseJson2Idiom(jsonStr string) main2.Idiom {
	idiom := main2.Idiom{}
	tempMap := make(map[string]interface{})
	json.Unmarshal([]byte(jsonStr), &tempMap)
...
//将go数据写出到json文件
func WriteIdioms2File(idiomsMap map[string]main2.Idiom, path string) {
...
//读入json文件为go数据
func ReadIdiomsFromFile(dstPath string) (idiomsMap map[string]main2.Idiom, err error) {
	idiomsMap = make(map[string]main2.Idiom)
